...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (!gameStarted || isShopOpen || isElevatorOpen || isGameOver || showTutorial || showSurfaceAnim) {
  ...
  }, [gameStarted, isShopOpen, isElevatorOpen, isGameOver, playerPos.y, showTutorial, showSurfaceAnim]);
  ...
  const handleElevatorTravel = (targetDepth: number) => {
    ...
    setIsBusy(true);
    setTimeout(() => {
      setPlayerPos(prev => ({ x: prev.x, y: targetDepth }));
      const radius = getVisionRadius(stats.scannerLevel);
      setGrid(prev => revealArea(prev, playerPos.x, targetDepth, radius));
  ...
  const checkUnlockElevator = (y: number) => {
    const foundDepth = ELEVATOR_DEPTHS.find(d => Math.abs(d - y) < 2); 
  ...
  const handleMove = useCallback((dx: number, dy: number) => {
    if (isShopOpen || isElevatorOpen || isGameOver || !gameStarted || showTutorial || isBusy || showSurfaceAnim) return;
    ...
    // MOVE TO EMPTY BLOCK (or PICK UP ITEM)
    ...
    if ((targetBlock.type as BlockType) !== BlockType.BEDROCK) {
      ...
      const validBlocks = targets
        ...
        .filter(item => item.block && item.block.type !== BlockType.BEDROCK && item.block.type !== BlockType.EMPTY);
  ...
  const checkExposure = (x: number, y: number): boolean => {
    ...
    for (const [dx, dy] of directions) {
        ...
        if (idx !== -1) {
            if (grid[idx].type === BlockType.EMPTY) return true;
        } else if (ny < 0) {
            return true;
        }
    }
  ...
  return (
    <div className="h-screen w-screen bg-[#0c0805] relative flex justify-center overflow-hidden select-none">
      ...
      {showSurfaceAnim && (
        <SurfaceTransition 
          ...
          onComplete={() => {
            ...
            // Auto-open shop disabled per request
          }}
        />
      )}
          ...
          {visibleRows.map(y => (
            <React.Fragment key={y}>
              {Array.from({ length: GRID_WIDTH }, (_, x) => {
                ...
                // Determine if mining target for animation
                // If multi-mining, we only animate the primary target tile for simplicity, or could animate all
                const isBeingMined = miningTarget?.x === x && miningTarget?.y === y; 
                // Or allow multi-shake? For now primary target only is cleaner.
      ...
      <div className="absolute bottom-8 right-8 flex flex-col gap-2 md:hidden opacity-80 z-40">
        <button className="p-4 bg-gray-800/80 rounded-lg text-white" onClick={() => handleMove(0, -1)}><ArrowUp /></button>
        <div className="flex gap-2">
          <button className="p-4 bg-gray-800/80 rounded-lg text-white" onClick={() => handleMove(-1, 0)}><ArrowLeft /></button>
          <button className="p-4 bg-gray-800/80 rounded-lg text-white" onClick={() => handleMove(0, 1)}><ArrowDown /></button>
          <button className="p-4 bg-gray-800/80 rounded-lg text-white" onClick={() => handleMove(1, 0)}><ArrowRight /></button>
        </div>
      </div>
    </div>
  );
