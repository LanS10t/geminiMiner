...
import { GRID_WIDTH, GRID_HEIGHT, DEPTH_TIERS, MINERALS, QUALITY_MULTIPLIERS } from '../constants';
...
export const generateGrid = (): BlockData[] => {
  ...
  for (let y = 0; y < GRID_HEIGHT; y++) {
    for (let x = 0; x < GRID_WIDTH; x++) {
      ...
      // REBALANCE: Threshold lowered to 0.35 and scale tweaked for better connectivity
      ...
      // 3. Vein Generation
      // Scale 0.25 creates patches of minerals.
...
export const revealArea = (grid: BlockData[], centerX: number, centerY: number, radius: number): BlockData[] => {
  ...
  // If the player is standing in an EMPTY block (Cave/Air), propagate visibility 
  // through the entire connected empty space and reveal the immediate solid walls.
  const centerIdx = getBlockIndex(centerX, centerY);
  if (centerIdx !== -1 && newGrid[centerIdx].type === BlockType.EMPTY) {
    ...
    while (queue.length > 0) {
      ...
      for (const n of neighbors) {
        ...
        if (nIdx !== -1 && !visited.has(nIdx)) {
          ...
          // Always reveal the neighbor (whether it's air or wall) 
          // because if we can see the air next to it, we can see the face of this block.
          ...
          // Only continue traversing through EMPTY blocks (light travels through air)
          if (neighborBlock.type === BlockType.EMPTY) {
             queue.push(nIdx);
          }
        }
      }
